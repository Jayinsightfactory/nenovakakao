...
- "sender": 발신자 이름 (정확히 표시된 그대로).
  ⚠️ 화면 '오른쪽'에 붙어 표시되는(이름표·프로필사진 없는) 말풍선은 '로그인 사용자 본인'이
  보낸 메시지다. 이 경우 sender 를 정확히 "나" 로 표기한다(빈 문자열로 두지 말 것).
  화면 '왼쪽' 말풍선은 그 위에 표시된 발신자 이름을 그대로 쓴다.
...
- ⚠️ content 는 메시지 본문 전체를 글자 그대로 끝까지 추출(요약·생략·말줄임표 금지). 길어도 전부.
- 이 이미지는 채팅 화면을 위→아래로 여러 장 이어붙인 것일 수 있다. 이음새 부근에서 같은 줄이
  중복으로 보일 수 있는데, 중복 줄은 한 번만 포함한다. 한 메시지가 이음새로 나뉘어 보이면
  하나의 메시지로 합쳐 완전한 content 로 만든다(중복·끊김 없이).
...
def _scroll_chat_up(hwnd, notches: int = 6) -> None:
    """채팅 패널(우측)을 위(과거)로 스크롤. ctypes 휠 — pyautogui fail-safe 회피."""
    import ctypes
    try:
        from core.window_manager import get_pos_tuple
        wl, wt, ww, wh = get_pos_tuple("kakaowork_main")
        ...
        return
    u = ctypes.windll.user32
    # 채팅패널 중앙: 캡처영역(dx345,w410 / dy95,h690) 중심 ≈ (+550, +440)
    try:
        u.SetCursorPos(int(wl + 550), int(wt + 440))
        time.sleep(0.08)
        for _ in range(max(1, notches)):
            u.mouse_event(0x0800, 0, 0, 120, 0)  # MOUSEEVENTF_WHEEL, +delta=위로(과거)
            time.sleep(0.02)
        time.sleep(0.35)
    except Exception:
        pass
...
def _img_signature(img):
    """대략 서명(32x32 grayscale 픽셀) — 캡처 변화/맨위도달 감지용."""
    try:
        return tuple(img.convert("L").resize((32, 32)).getdata())
    except Exception:
        return tuple()


def _sig_diff(a, b) -> int:
    if not a or not b or len(a) != len(b):
        return 9999
    return sum(1 for x, y in zip(a, b) if abs(x - y) > 16)


def capture_chat_panel_stitched(hwnd, out_path: Path | None = None, *, max_extra: int = 2) -> Path:
    """채팅패널을 위로 스크롤하며 여러 장 캡처 → 세로로 이어붙인 1장 반환.

    긴 메시지가 한 뷰포트(≈690px)를 넘어 잘리는 문제 해결(2026-06-05).
    - 맨 아래(최신) 1장 + 위로 스크롤하며 최대 max_extra 장.
    - '변화 거의 없음'이면 맨 위 도달로 보고 조기 종료(짧은 대화는 1~2장만 → 빠름).
    - 스티치 순서: 위(과거)=맨 마지막 캡처 → 아래(최신)=첫 캡처.
    - 이음새 약간 겹침(누락 방지) → Vision 이 중복 줄 병합(PROMPT 지시).
    너무 길면(Claude 이미지 1568px 리사이즈) 글자 흐려질 수 있어 max_extra 보수적(2=3장).
    """
    from PIL import Image
    CAPTURES.mkdir(parents=True, exist_ok=True)
    if out_path is None:
        out_path = CAPTURES / f"kw_stitch_{int(time.time()*1000)}.png"

    def _cap_once():
        p = CAPTURES / f"_st_{int(time.time()*1000)}.png"
        if not capture_region(hwnd, "kakaowork_chatpanel", p):
            p = capture_chat_panel(hwnd, p, right_panel_only=True)
        return Image.open(p).copy(), p

    imgs, tmps = [], []
    try:
        img0, p0 = _cap_once(); imgs.append(img0); tmps.append(p0)
        last = _img_signature(img0)
        for _ in range(max(0, max_extra)):
            _scroll_chat_up(hwnd, notches=6)
            img, p = _cap_once(); tmps.append(p)
            sig = _img_signature(img)
            if _sig_diff(sig, last) < 20:   # 거의 변화 없음 → 맨 위 도달/스크롤 불가
                break
            imgs.append(img); last = sig
        ordered = list(reversed(imgs))     # 위(과거)→아래(최신)
        if len(ordered) == 1:
            ordered[0].save(out_path)
        else:
            W = max(im.width for im in ordered)
            H = sum(im.height for im in ordered)
        ...
        if len(imgs) > 1:
            print(f"  [WORK-VISION] 스티치 {len(imgs)}장 (긴 메시지 대응)", flush=True)
        return out_path
    finally:
        for p in tmps:
            try: p.unlink()
